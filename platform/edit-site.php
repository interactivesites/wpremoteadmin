...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($url) || empty($api_token)) {
        ...
        if (!filter_var($url, FILTER_VALIDATE_URL)) {
            ...
            if ($db->update_site($id, $name, $url, $api_token)) {
                // Handle contract data (save if contract_name provided or if updating existing contract)
                if (!empty($_POST['contract_name']) || $contract) {
                    // Handle file upload
                    ...
                    if (isset($_FILES['contract_file']) && $_FILES['contract_file']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if ($file_extension === 'pdf') {
                            $file_name = 'contract_' . $id . '_' . time() . '.pdf';
                    ...
                    $db->save_contract($id, $contract_data);
                }
...
<head>
    ...
    <title>Edit Site - WordPress Update Manager</title>
        ...
        <div class="form-container">
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" required value="<?php echo htmlspecialchars($site['name']); ?>">
                    <small>Friendly name to identify this site</small>
                </div>
                
                <div class="form-group">
                    <label for="url">Site URL *</label>
                    <input type="url" id="url" name="url" required value="<?php echo htmlspecialchars($site['url']); ?>" placeholder="https://example.com">
                    <small>Full URL of your WordPress site</small>
                </div>
                
                <div class="form-group">
                    <label for="api_token">API Token *</label>
                    <input type="text" id="api_token" name="api_token" required value="<?php echo htmlspecialchars($site['api_token']); ?>" placeholder="Your API token from WordPress plugin">
                    <small>Generate this token in WordPress admin under Settings > Remote Updates</small>
                </div>
                
                <div class="form-section">
                    <h2>Hosting Contract</h2>
                    
                    <div class="form-group">
                        <label for="contract_name">Contract Name</label>
                        <input type="text" id="contract_name" name="contract_name" value="<?php echo htmlspecialchars($form_data['contract_name']); ?>">
                    </div>
                    
                    <div class="form-group">
                        <label for="start_date">Start Date</label>
                        <input type="date" id="start_date" name="start_date" value="<?php echo htmlspecialchars($form_data['start_date']); ?>" onchange="calculateYearlyPrice()">
                    </div>
                    
                    <div class="form-group">
                        <label for="end_date">End Date (Optional)</label>
                        <input type="date" id="end_date" name="end_date" value="<?php echo htmlspecialchars($form_data['end_date'] ?? ''); ?>">
                    </div>
                    
                    <div class="form-group">
                        <label for="monthly_price">Monthly Price</label>
                        <input type="number" id="monthly_price" name="monthly_price" step="0.01" value="<?php echo htmlspecialchars($form_data['monthly_price']); ?>" onchange="calculateYearlyPrice()">
                    </div>
                    
                    <div class="form-group">
                        <label for="yearly_price">Yearly Price</label>
                        <input type="number" id="yearly_price" name="yearly_price" step="0.01" value="<?php echo htmlspecialchars($form_data['yearly_price']); ?>" onchange="calculateMonthlyPrice()">
                        <small>Auto-calculated from monthly price if left empty</small>
                    </div>
                    
                    <div class="form-group">
                        <label>
                            <input type="checkbox" name="payment_flag_current_year" value="1" <?php echo ($form_data['payment_flag_current_year'] ?? 0) ? 'checked' : ''; ?>>
